...
const width = window.innerWidth * 0.45, // width of svg = half of browser window
...
const hwidth = window.innerWidth * 0.5,
  hheight = window.innerHeight / 1.25,
  hpaddingInner = 0.2,
  hmargin = { top: 30, bottom: 100, left: 105, right: 40 };
...
const hrect = hsvg
  ...
  .data(data)
  .join("rect")
    // .attr("x", d => d3.min(data, d => d.amount)) // https://stackoverflow.com/questions/25047562/d3-js-horizontal-bar-graph-change-bar-direction-left-to-right-instead-of-righ
    // .attr("y", d => hyScale(d.state))
    // .attr("width", d => hxScale(d.amount))
    // .attr("height", hyScale.bandwidth())
    // .attr("fill", d => hbarColor(d.amount));
    .attr("x", d => hxScale(d3.min(data, d => d.amount))) // https://stackoverflow.com/questions/25047562/d3-js-horizontal-bar-graph-change-bar-direction-left-to-right-instead-of-righ
    .attr("y", d => hyScale(d.state))
    .attr("width", d => d.amount + margin.left)
    .attr("height", hyScale.bandwidth())
    .attr("fill", d => hbarColor(d.amount));
...
const htext = hsvg
  ...
  //.attr("class", "label")
    .attr("fill", d => hbarColor(d.amount))
    .attr("x", d => d.amount + 150)
    .attr("y", d => hyScale(d.state) + (hyScale.bandwidth() / 2))
  .text(d => d.amount)
